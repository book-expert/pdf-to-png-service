...
func run(ctx context.Context) error {
	...
	appLogger.Infof(
		"Worker is running, listening for jobs on '%s'வுகளை",
...
func (j *job) analyzePDF(ctx context.Context) error {
	...
	if j.event.Settings != nil {
		if j.event.Settings.Scene != "" {
			input.Scene = j.event.Settings.Scene
		}
		if j.event.Settings.Style != "" {
			input.Style = j.event.Settings.Style
		}
		if j.event.Settings.Accent != "" {
			input.Accent = j.event.Settings.Accent
		}
		if j.event.Settings.Articulation != "" {
			input.Articulation = j.event.Settings.Articulation
		}
		if j.event.Settings.Pace != "" {
			input.Pace = j.event.Settings.Pace
		}
		if j.event.Settings.Personality != "" {
			input.Personality = j.event.Settings.Personality
		}
		if j.event.Settings.Exclusions != "" {
			input.Exclusions = j.event.Settings.Exclusions
		}
	}

	config, err := j.analyzer.AnalyzePDF(ctx, j.pdfData, input)
	if err != nil {
		return fmt.Errorf("gemini analysis: %w", err)
	}

	config.SourceDocumentID = j.event.PDFKey
	j.appLogger.Infof("Analysis Complete. Session ID: %s", config.SessionID)
	...
	// Also, if VoiceID was generated by Gemini, we might want to override the requested voice?
	// The plan says: "Result: A structured AudioSessionConfig containing... VoiceID".
	// If the user requested a specific voice in JobSettings, we should probably keep it unless it's empty.
	if j.event.Settings.Voice == "" && config.VoiceID != "" {
		j.event.Settings.Voice = config.VoiceID
	}
